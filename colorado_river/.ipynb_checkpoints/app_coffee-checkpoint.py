...
# ---------- App ----------
# Bin numeric ratings into classes for classification models
...
with tab_classify:
    ...
    feature_cols = [c for c in feature_cols if filtered[c].notna().any()]
    if len(feature_cols) < 2 or "rating" not in filtered.columns:
        st.info("Need at least two predictor columns and rating to fit classifiers.")
    else:
        cls_data = filtered[feature_cols + ["rating"]].dropna()
        if len(cls_data) < 50:
            st.info("Not enough rows after filtering to train/test classifiers.")
        else:
            X = cls_data[feature_cols]
            y = cls_data["rating"].apply(bin_rating)
            ...
            def report_to_df(y_true, y_pred, label: str) -> pd.DataFrame:
                rep = classification_report(y_true, y_pred, output_dict=True, zero_division=0)
                df_rep = pd.DataFrame(rep).T.reset_index().rename(columns={"index": "label"})
                df_rep["model"] = label
                return df_rep

            rep_knn = report_to_df(y_test, y_pred_knn, "KNN")
            rep_tree = report_to_df(y_test, y_pred_tree, "Tree")
            rep_all = pd.concat([rep_knn, rep_tree], ignore_index=True)

            st.markdown(f"**Features used:** {', '.join(feature_cols)}")
            st.markdown(f"**Train/Test samples:** {len(X_train)}/{len(X_test)} (stratified by class)")

            show_cols = ["model","label","precision","recall","f1-score","support"]
            st.dataframe(rep_all[show_cols].round({"precision":3,"recall":3,"f1-score":3}), hide_index=True, use_container_width=True)

            def plot_conf(y_true, y_pred, title):
                labels = sorted(y_test.unique())
                cm = confusion_matrix(y_true, y_pred, labels=labels)
                cm_df = pd.DataFrame(cm, index=labels, columns=labels).reset_index().melt(id_vars="index", var_name="Pred", value_name="Count")
                return alt.Chart(cm_df).mark_rect().encode(
                    x=alt.X("Pred:N", title="Predicted"),
                    y=alt.Y("index:N", title="Actual"),
                    color=alt.Color("Count:Q", scale=alt.Scale(scheme="blues")),
                    tooltip=["index","Pred","Count"]
